...
public class SoundSystem : Entity {
    ...
    [Space(5)]
    [SerializeField] AudioMixerGroup sfxMixer, musicMixer;
    ...
    public override void Tick() {
        ...
        foreach (var request in SFXRequests) {
            request.unit.volume = SFXVolume * masterVolume;
        }
    }
    ...
    //User
    public bool PlayLocalSFX(string key, GameObject caller) {
        if (!canPlaySFX) {
            Warning("Unable to play local sfx associated with key [" + key + "]\n Reason: SoundSystem is not able to play SFX audio clips!");
            return false;
        }
        AudioSource audioSource = caller.AddComponent<AudioSource>();
        soundUnits.Add(audioSource);
        SFXRequest request = new SFXRequest();
        request.owner = caller;
        request.key = key;
        request.unit = audioSource;
        SFXRequests.Add(request);
        audioSource.clip = loadedSFXAssetsHandles[key].Result;
    ...
    public bool PlayTrack(string key) {
        ...
        trackAudioSource.clip = loadedTracksAssets[key].Result;
        trackAudioSource.outputAudioMixerGroup = musicMixer;
